...
<template>
  <div class="flex min-h-0 flex-col gap-5 overflow-y-auto pb-6">
    ...
    <template v-else-if="detail.data.value">
      <header class="flex flex-wrap items-end justify-between gap-3"><div><p class="text-sm text-muted-foreground">Закупки / Прайс-листы</p><div class="flex items-center gap-2"><h1 class="text-2xl font-semibold tracking-tight">{{ detail.data.value.title }}</h1><Badge variant="outline">{{ detail.data.value.status }}</Badge></div><p class="mt-1 text-sm text-muted-foreground">{{ detail.data.value.source_url_display }}</p></div><div class="flex gap-2"><Button v-if="detail.data.value.status === 'active'" variant="outline" :disabled="action.isPending.value" @click="action.mutate({ id, action: 'sync' })"><RefreshCw />Загрузить сейчас</Button><Button v-if="detail.data.value.status === 'active'" variant="outline" @click="action.mutate({ id, action: 'pause' })"><Pause />Пауза</Button><Button v-if="detail.data.value.status === 'paused'" @click="action.mutate({ id, action: 'resume' })"><Play />Возобновить</Button></div></header>
