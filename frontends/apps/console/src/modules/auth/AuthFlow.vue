...
import { AuthProviderButton, OtpCodeInput } from "@/modules/auth/components";
...
const canRequestOtp = computed(
  () => email.value.trim().length > 3 && !sessionStore.isRequestingOtp,
);
const canConfirmOtp = computed(
  () => code.value.length === 6 && !sessionStore.isConfirmingOtp,
);
...
<template>
  ...
  <Card v-else-if="loginStep === 'workspace-not-found'" class="w-full max-w-sm">
    <CardHeader class="text-center">
      ...
      <CardDescription
        >This workspace was not found or is not available for
  ...
  <Card v-else class="w-full max-w-sm">
    <CardHeader class="items-center text-center">
      ...
      <CardTitle class="text-base">Check your email</CardTitle>
      <CardDescription>
