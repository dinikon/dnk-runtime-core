...
from src.modules.communication.application.template.dto import TemplateVersionDTO
...
class ActivateTemplateVersionUseCaseProtocol(Protocol):
    """Порт use case активации версии шаблона."""

    async def __call__(
        self, command: ActivateTemplateVersionCommand
    ) -> TemplateVersionDTO:
        """Активирует версию шаблона и возвращает DTO."""
        ...


class ActivateTemplateVersionUseCase:
    """Use case активации версии шаблона через доменный сервис."""
    ...
    async def __call__(
        ...
        """Выполняет команду активации версии и мапит entity в DTO."""
        version = await self._service.activate_template_version(
...
__all__ = [
    ...
    "ActivateTemplateVersionUseCaseProtocol",
]
