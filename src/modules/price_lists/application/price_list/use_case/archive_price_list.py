...
class ArchivePriceListUseCase(PriceListUseCase):
    """Выполняет действие archive_price_list через внедрённые порты."""

    async def __call__(self, command: ArchivePriceListCommand) -> ActionDTO:
        price = await self.repository.get(
            command.tenant_id, command.price_list_id, for_update=True
        )
        now = self.clock.now()
        price.transition("archive", command.actor_id, now)
        job_id = None
        canceled = None
        ...
        return ActionDTO(status=price.status, job_id=job_id, canceled_jobs=canceled)


__all__ = ["ArchivePriceListUseCase"]
