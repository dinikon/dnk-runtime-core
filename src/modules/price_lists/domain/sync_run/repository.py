...
class SyncRunRepository(Protocol):
    ...
    async def unfinished_batch(
        self, tenant_id: EntityIdVO, after: SyncRunIdVO | None
    ) -> list[SyncRun]:
        """Читает активные runs ограниченным keyset-пакетом."""
        ...

    async def skip_running(
        self, tenant_id: EntityIdVO, price_list_id: PriceListIdVO, now: datetime
    ) -> None:
