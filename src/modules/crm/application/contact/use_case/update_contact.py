...
from src.modules.crm.application.contact.integration_events import contact_updated_event
...
class UpdateContactUseCase:
    ...
    async def __call__(self, command: RenameContactCommand) -> ContactDTO:
        ...
        after = _contact_data(contact)
        changed_fields = _changed_fields(before=before, after=after)
        if changed_fields:
...
def _changed_fields(*, before: dict, after: dict) -> list[str]:
    return [
        field
        for field in ("first_name", "last_name", "middle_name", "status", "tags")
