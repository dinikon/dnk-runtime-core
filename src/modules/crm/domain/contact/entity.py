...
from modules.crm.domain.contact_point.entity import ContactPoint

from modules.crm.domain.contact.value_objects import ContactId, PersonName
...
class Contact:
    ...
    def get_contact_point(self, point_id: ContactPointId) -> ContactPoint:
        for point in self.contact_points:
            if point.id == point_id:
                return point
        raise ValueError(f"Contact point not found: {point_id}")
    ...
    def websites(self) -> list[ContactPoint]:
        return self.get_points_by_kind(ContactPointKind.website())
