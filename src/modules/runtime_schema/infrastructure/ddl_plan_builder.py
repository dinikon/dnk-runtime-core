...
class DdlPlanBuilder(DdlPlanBuilderProtocol):
    ...
    def build(self, *, schema: str, diff: DdlDiff) -> DdlPlan:
        ...
        for item in diff.tables_to_create:
        ...
        for item in diff.columns_to_add:
            sql = (
                ...
                f"ADD COLUMN {self._build_column_sql(item.column)}"
            )
            operations.append(
                DdlOperation(
    ...
    def _build_create_table_sql(
        ...
        *,
        ...
        column_sql = ", ".join(self._build_column_sql(column) for column in columns)
        return (
            f"CREATE TABLE IF NOT EXISTS {self._qualified_table(schema=schema, table_name=table_name)} "
            f"({column_sql})"
        )

    def _build_column_sql(self, column: ColumnSpec) -> str:
        ...
        reference_sql = ""
        if column.references_table and column.references_column and self._dialect_name != "sqlite":
            reference_sql = (
                f" REFERENCES {self._quote_identifier(column.references_table)}"
                f"({self._quote_identifier(column.references_column)})"
            )
            if column.on_delete:
                reference_sql += f" ON DELETE {column.on_delete.upper()}"
        return (
