...
class _InMemoryTemplateRepository:
    ...
    async def get_by_id(self, *, template_id):  # pragma: no cover - helper
...
class _InMemoryLinkRepository:
    ...
    async def get_by_id(self, *, domain_id, link_id):  # pragma: no cover - helper
        for item in self.items:
            if item.domain_id == domain_id and item.id == link_id:
                return item
        return None


class TestTemplateCreationService(unittest.IsolatedAsyncioTestCase):
    async def test_create_with_explicit_code(self) -> None:
    ...
    async def test_create_with_duplicate_explicit_code_raises(self) -> None:
        domain_id = EntityIdVO.from_value(uuid4())
    ...
    async def test_create_without_code_generates_until_unique(self) -> None:
        domain_id = EntityIdVO.from_value(uuid4())
        checker = _InMemoryLinkUniquenessChecker(
            taken={
    ...
    async def test_create_service_rejects_invalid_attempts_limit(self) -> None:
        checker = _InMemoryLinkUniquenessChecker()
...
class TestCreateTemplateUseCase(unittest.IsolatedAsyncioTestCase):
    async def test_execute_returns_dto_and_persists_entities(self) -> None:
        checker = _InMemoryLinkUniquenessChecker()
