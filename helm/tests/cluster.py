...
from test_render import HELM, ROOT, runtime_values
    ...
    def __enter__(self):
        ...
        if not self.published and not self.reuse_test_images:
            for dockerfile, cwd, image, target in [
                ("Dockerfile", ROOT, self.images[0], None),
                ("Dockerfile", ROOT / "frontends", self.images[1], None),
            ]:
                run(
                    [
                        "docker",
        ...
        for image in self.images + INFRA_IMAGES:
            if run(
                ...
                if image in self.images:
                    raise RuntimeError(
                        "Local integration image is unavailable: " + image
                    )
