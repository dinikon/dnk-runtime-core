...
# 65. Согласованные уточнения реализации (2026-09-21)

Этот раздел уточняет проектные примеры выше и имеет приоритет при различии деталей.
Структура и правила кода — `docs/develop-style.md`; описание публичного API и развёртывания —
`docs/modules/currency.md`.

- Политика, разрешённые валюты, timezone и первый период появляются только после явной
  первоначальной настройки администратором в одной UoW. До сохранения UAH, НБУ,
  PREVIOUS_AVAILABLE, HALF_UP и Europe/Kyiv являются только значениями формы.
- Currency принимает готовую бизнес-дату. Публичный `CurrencySettingsReader` возвращает
  immutable DTO настроек. Потребитель применяет `BusinessCalendar`; timezone и версия
  политики фиксируются на время публикации импорта, включая несколько пакетов.
- `Money.__truediv__` выполняет точное конечное деление независимо от внешнего Decimal context.
  Бесконечная десятичная дробь требует явного `divide(precision=..., rounding=...)`.
  Inverse/cross используют 38 значащих цифр и режим округления политики.
- `MoneyQuantizer`: CALCULATION сохраняет промежуточное значение; BOOKING требует явной
  точности или minor_units; DISPLAY без обоих сохраняет точность исходного значения.
  HTTP precision означает число знаков после запятой. Деньги передаются строками.
- Facade возвращает application DTO, включая типизированные successful/unavailable
  пакетные результаты. Инфраструктурные ошибки остаются исключениями. Identity-конверсия
  тоже создаёт снимок. Новые снимки сохраняют параметры округления; legacy JSON читается
  без перезаписи и без выдуманных значений отсутствующих полей.
- Организация хранит nullable `default_display_currency`, Identity хранит nullable
  `display_currency` пользователя. Приоритет: личная → явная организации → сегодняшняя
  основная. Null — наследование. Явная валюта организации защищена от отключения; личная
  не блокирует отключение и остаётся видимой как недоступное предпочтение.
- Предложения хранят исторический снимок только при новом OfferState. Текущий результат
  в основной валюте (`current_conversion`) и пользовательский (`display_conversion`)
  читаются отдельно. При совпадении целей результат переиспользуется. История не достраивается,
  исходные NUMERIC(19,4) цены и хеши состояний не меняются.
- Одинаковая политика, разрешение валюты или ручной курс не меняют timestamps и не создают
  событий/ревизий; expected_version проверяется даже для сохранения без изменений.
- `FunctionalCurrencyActivated`: задание создаётся в той же UoW, первый уже наступивший
  период активируется при настройке. Worker проверяет актуальный timezone и атомарно
  сохраняет маркер с outbox-событием. Повтор не создаёт событие; задержка сохраняет дату
  действия и фактическое время. Изменение timezone переносит будущие задания.
- `RateResolutionFailed`: отдельная audit-UoW атомарно пишет диагностику и outbox.
  Дедупликация — операция, tenant, пара, дата, источник, версия политики и причина.
  Для импорта операция равна SyncRunId, для HTTP генерируется сервером. Когда политики
  нет, историческая бизнес-дата неизвестна и остаётся null; исходная цена сохраняется.
  Ошибки БД и программы не считаются отсутствием курса.
- Регистрация источников единая для policy validation, Console и CLI. MANUAL локальный,
  НБУ внешний; подмена источников запрещена. Глобальные импорты имеют только global audit.
- Агрегаты и файлы разделены предметно: directory, policy, enabled_currency,
  functional_currency, manual_rate, provider_rate, rate_import, resolution_failure.
  Resolver, конвертация и registry остаются сервисами/контрактами. HTTP имеет отдельный
  controller на сценарий и явные локальные except; универсальные wire/error wrappers исключены.
- Применённые миграции не переписываются. Новая tenant revision добавляет предпочтения,
  активацию и аудит; межсхемные FK отсутствуют. Репозитории используют общую UoW session,
  Core и schema_translate_map и не выполняют commit.
- Развёртывание: global migrations → tenant migrations → приложение и worker →
  `dnk-manage currency restore-activations` → явная настройка → загрузка курсов → включение
  выключенного по умолчанию global CronJob. Restore-команда идемпотентна.
- Будущие Supply/Order/Payment, дополнительные провайдеры, Redis, tenant-specific provider
  storage и динамические плагины остаются расширениями.
