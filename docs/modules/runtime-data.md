...
находятся в потребляющих модулях (`crm`, `inventory`, `custom_object`, `communication`).
...
- `src/modules/inventory/presentation/depends/infrastructure.py` создает `RuntimeFieldTypePolicy`,
...
| `inventory`               | внешний потребитель             | Использует runtime gateways и runtime errors для product/category runtime repositories/controllers.                                   |
...
Внутри `runtime_data` нет зависимости на `crm`, `inventory`, `custom_object` или `communication`; эти зависимости
...
- Текущая структура намеренно отличается от эталонного aggregate-модуля `inventory`: нет `domain/<aggregate>/entity.py`,
