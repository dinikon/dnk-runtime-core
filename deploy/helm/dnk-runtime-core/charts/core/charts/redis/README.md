...
its StatefulSet. It is intended for the Core chart and can also be installed on
...
default is `<release>-core-redis`. Inline passwords create `<fullname>-auth` with
...
Coordinate password changes with the Core backend. The default image has 16
logical databases, numbered 0–15; Core uses database 1 by default.
