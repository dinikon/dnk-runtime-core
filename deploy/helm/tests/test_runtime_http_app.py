...
PATH = Path(__file__).resolve().parents[1] / "dnk-platform/charts/dnk-runtime-core/files/http_app.py"
...
class SessionCookieTests(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_https_session_preserves_value_expiry_and_other_cookies(self):
        headers = [
            (b"set-cookie", b"dnk_session=opaque-token; Max-Age=300; Path=/; HttpOnly; SameSite=lax"),
            (b"set-cookie", b"other_session=unchanged; Path=/"),
            (b"content-type", b"application/json"),
        ]
        result = await self.response(headers)
        self.assertEqual(result[1:], headers[1:])
        self.assertEqual(result[0][1], b"dnk_session=opaque-token; Max-Age=300; Path=/; Secure; HttpOnly; SameSite=Lax")
    ...
    async def test_logout_preserves_deletion_and_deduplicates_flags(self):
        result = await self.response([
            (b"Set-Cookie", b'dnk_session=""; expires=Thu, 01 Jan 1970 00:00:00 GMT; Max-Age=0; Path=/; Secure; SameSite=None; HttpOnly')
        ])
        value = result[0][1]
    ...
    async def test_exact_cookie_name_and_non_cookie_headers(self):
        headers = [(b"set-cookie", b"dnk_session_extra=abc"), (b"x-cookie", b"dnk_session=abc")]
