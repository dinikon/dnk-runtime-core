...
class TelegramGatewayClient:
    """Deliver allauth-owned numeric codes through the Telegram Gateway API."""

    endpoint = "https://gatewayapi.telegram.org/sendVerificationMessage"

    def send_verification_code(
        self, phone: str, code: str, *, ttl: int | None = None
    ) -> str:
        """Send a validated six-digit code with bounded transport and no redirects."""
        if ttl is None:
            ttl = settings.ACCOUNT_PHONE_VERIFICATION_TIMEOUT
